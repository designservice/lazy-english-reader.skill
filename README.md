...
| 产物 | 适合什么时候用 | 大致耗时 |
|------|----------------|----------|
| 🔁 **全本中文译本** | 想留一份可检索、可导出的中文版本 | 1-3 小时 |
| 📖 **章节精读笔记** | 想真正读懂每一章，并在 Obsidian 里继续批注 | 5-15 min/章 |
| 🌐 **主题索引** | 读完整本后，按人物、概念、方法论回看全书 | 30-60 min |
...
这个 skill 不绑定 Claude Code。仓库里已经放了几个轻量适配文件，让其他 agent 更容易读懂同一套流程：
...
这些 adapter 不复制整套说明，只告诉对应 agent 去读 `SKILL.md`，并把“读写文件、运行命令、提问、后台任务”映射到自己的工具。真正的流程仍然只维护在 `SKILL.md` 里。

`scripts/*.py` 可以直接命令行调用，`templates/*.md` 是普通 Markdown 模板。生成的 `CLAUDE.md` 是这本书的 AI 上下文文件；如果你的 agent 使用 `AGENTS.md`、`GEMINI.md` 或其他约定，可以复用同一份内容。
